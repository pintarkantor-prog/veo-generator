    ...
    with st.expander(f"KONFIGURASI ADEGAN {idx_s}", expanded=is_leader):
        ...
        with c_light:
            ...
            if is_leader:
                ...
                l_val = st.selectbox(f"Pencahayaan {idx_s}", options_lighting, key=f"light_{idx_s}")
        ...
        for i in range(0, num_chars, 2):
            ...
            if c_idx + 1 < num_chars:
                c_idx2 = i + 1
                name2 = char_list[c_idx2]["name"] if char_list[c_idx2]["name"] else f"Karakter {c_idx2+1}"
                with cols[2]: co2 = st.selectbox(f"Kondisi {name2}", options_cond, key=f"cond_{c_idx2}_{idx_s}")
