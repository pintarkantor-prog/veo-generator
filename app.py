...
def tampilkan_area_staf():
    ...
    with t5:
        ...
        # --- KONEKSI DATA USER ---
        # Menggunakan 'user_aktif' sesuai sistem login lo
        ...
        # --- KONSTRUKSI HTML (FULL TEXT LO - NO CUT) ---
                ...
                <tr>
                    <td style="width: 50%;">PIHAK KEDUA,<br><br><br>
                        <span style="color:green; font-weight:bold;">
                            {"[ E-SIGNED VERIFIED: " + now.strftime('%H:%M:%S') + " ]" if is_signed else "(BELUM TANDA TANGAN)"}
                        </span><br>
        ...
        # --- HEADER UTAMA ---
        st.subheader("📜 Pengesahan Kontrak Kerja Digital")
        ...
        if user_login == "dian":
            ...
            # STATUS PREVIEW DI SESSION
            if f'preview_done_{user_login}' not in st.session_state:
                st.session_state[f'preview_done_{user_login}'] = False

            col1, col2 = st.columns([1, 1])
            with col1:
                # TOMBOL PREVIEW (WAJIB KLIK)
                if st.button("🔍 PREVIEW KONTRAK (PRINT VIEW)", use_container_width=True):
                    st.session_state[f'preview_done_{user_login}'] = True
                    st.components.v1.html(html_kontrak_preview + "<script>window.print();</script>", height=0)
            
            with col2:
                if st.session_state[f'preview_done_{user_login}']:
                    st.success("✅ Preview Selesai.")
                else:
                    st.info("👈 Klik Preview Dulu.")

            # TAMPILKAN DI LAYAR JUGA
            st.components.v1.html(html_kontrak_preview, height=600, scrolling=True)

            # LOGIKA CHEKBOX HANYA MUNCUL JIKA SUDAH PREVIEW
            if st.session_state[f'preview_done_{user_login}']:
                setuju_syarat = st.checkbox(f"Saya, {nama_staff_resmi}, menyatakan telah membaca dan menyetujui seluruh isi kontrak di atas.")
                ...
                if setuju_syarat:
                    ...
                    st.button("✅ SAHKAN TANDA TANGAN DIGITAL", disabled=True, use_container_width=True)
            else:
                st.warning("Silakan klik tombol 'PREVIEW KONTRAK' di atas untuk mengaktifkan persetujuan.")
