...
def tampilkan_database_channel():
    ...
    with tab_jadwal:
        ...
        if df_j.empty:
            ...
            list_hp_tim1 = [h for h in df_display['HP'].unique() if pd.to_numeric(h, errors='coerce') <= 16]
            list_hp_tim2 = [h for h in df_display['HP'].unique() if pd.to_numeric(h, errors='coerce') > 16]
            
            # Gabungkan jadi satu list besar tapi kita kasih pembatas (marker)
            # Ini biar kodenya mirip struktur awal lo yang pake loop
            kelompok_tim = [
                {"nama": "LISA (HP 1-16)", "list": list_hp_tim1},
                {"nama": "INGGI (HP 17-23)", "list": list_hp_tim2}
            ]
