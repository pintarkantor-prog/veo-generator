...
# ==============================================================================
# Menggunakan Link i.ibb.co yang sudah Dian ganti agar AI langsung mengenali wajah.
LINK_REFERENSI = {
...
with c_header1:
    ...
    st.info(f"Staf Aktif: {st.session_state.active_user} | Fokus: Ketajaman Visual & Akurasi Wajah 🚀❤️")
with c_header2:
    if st.button("Logout 🚪"):
        st.session_state.logged_in = False
        st.rerun()
...
# ==============================================================================
# 12. GENERATOR PROMPT (PEMISAHAN TUGAS - TELANJANG LINK)
# ==============================================================================
if st.button("🚀 GENERATE ALL PROMPTS", type="primary"):
    ...
    if not active_scenes:
        ...
        # Link telanjang di paling depan (IMAGE PROMPT MODE)
        ...
        for item in active_scenes:
            ...
            # --- DNA INJECTION (STRICT FACE PRIORITY) ---
            dna_str = ""
            for idx, c in enumerate(all_chars_list):
                if c['name'].lower() in vis_lower:
                    # Menggunakan referensi nomor link agar AI tidak tertukar
                    dna_str += f"STRICT CHARACTER IDENTITY: Render {c['name']} using the exact human facial features shown in image reference link #{idx+1}. "

            st.subheader(f"✅ Hasil Adegan {item['num']}")
            # HASIL AKHIR: [URL] [URL] IMPORTANT: ... DNA ... Visual Scene ...
            img_final = f"{ref_links_only}{master_lock_instruction if item['num']==1 else ''}STATIC photograph, 9:16. {dna_str} Visual scene: {vis_core}. {a_cmd} {l_cmd} {img_quality_base} --ar 9:16"
            vid_final = f"{ref_links_only}{master_lock_instruction if item['num']==1 else ''}9:16 vertical video. {shot_map.get(item['shot'], 'Medium')} perspective. {dna_str} Visual scene: {vis_core}. {e_cam_move}. {l_cmd} {vid_quality_base}"
...
st.sidebar.markdown("---")
st.sidebar.caption("PINTAR MEDIA | V.1.3.13")
