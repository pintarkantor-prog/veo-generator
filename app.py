...
# ==============================================================================
# Menggunakan Direct Link i.ibb.co agar AI langsung mengunci wajah manusia asli.
LINK_REFERENSI = {
...
with c_header1:
    ...
    st.info(f"Staf Aktif: {st.session_state.active_user} | Storyboard Precision Mode V.1.3.14 🚀❤️")
with c_header2:
    if st.button("Logout 🚪"):
        st.session_state.logged_in = False
        st.rerun()
...
# ==============================================================================
# 9. PARAMETER KUALITAS (FULL EXPLICIT V.1.1.8)
# ==============================================================================
sharp_natural_stack = (
...
# ==============================================================================
# 12. GENERATOR PROMPT (THE PRECISION MAPPING - V.1.3.14)
# ==============================================================================
if st.button("🚀 GENERATE ALL PROMPTS", type="primary"):
    ...
    if not active_scenes:
        ...
        # TEKNIK BARU: Mapping satu per satu link di depan dengan label eksplisit
        ref_header = "Character Face Mapping: " + " ".join([f"({c['name']}) [ {c['url']} ]" for c in all_chars_list if c['url']]) + ". "
        
        char_defs_full = ", ".join([f"Karakter {idx+1} ({c['name']}: {c['desc']})" for idx, c in enumerate(all_chars_list) if c['name']])
        master_lock_instruction = f"IMPORTANT: Use exactly Character Face Mapping links for facial identity. Maintain physical traits: {char_defs_full}. "
        ...
        for item in active_scenes:
            ...
            if camera_map.get(item["cam"]) == "AUTO_MOOD":
                if any(x in vis_lower for x in ["lari", "jalan", "mobil"]): e_cam_move = "Dynamic Tracking Shot, follow subject motion"
            ...
            if "Bening" in l_type:
                ...
                l_cmd, a_cmd = "Intense moody overcast lighting, vivid recovery.", "Gray-cobalt sky with heavy thick wispy clouds."
            else:
                l_cmd, a_cmd = "Natural cinematic lighting.", "Clear 8k definition, zero digital noise."

            # DNA INJECTION (STRICT MAPPING)
            dna_injection = ""
            for c in all_chars_list:
                if c['name'].lower() in vis_lower:
                    # Injeksi perintah untuk mengambil wajah dari Link Spesifik miliknya
                    dna_injection += f"STRICT CHARACTER IDENTITY: Render {c['name']} using only the facial features from the face mapping link associated with their name. "

            st.subheader(f"✅ Hasil Adegan {item['num']}")
            # HASIL AKHIR: [Face Mapping] [Master Lock] [DNA] Visual Scene...
            img_final = f"{ref_header}{master_lock_instruction if item['num']==1 else ''}STATIC photograph, 9:16. {dna_injection} Visual scene: {vis_core}. {a_cmd} {l_cmd} {img_quality_base} --ar 9:16"
            vid_final = f"{ref_header}{master_lock_instruction if item['num']==1 else ''}9:16 vertical video. {shot_map.get(item['shot'], 'Medium')} perspective. {dna_injection} Visual scene: {vis_core}. {e_cam_move}. {l_cmd} {vid_quality_base}"
...
st.sidebar.markdown("---")
st.sidebar.caption("PINTAR MEDIA | V.1.3.14")
