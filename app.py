...
def tampilkan_kendali_tim():    
    ...
    daftar_bulan = {1: "Januari", 2: "Februari", 3: "Maret", 4: "April", 5: "Mei", 6: "Juni", 7: "Juli", 8: "Agustus", 9: "September", 10: "Oktober", 11: "November", 12: "Desember"}
    pilihan_nama = c_bln.selectbox("📅 Pilih Bulan Laporan:", list(daftar_bulan.values()), index=sekarang_wib.month - 1)
    bulan_dipilih = [k for k, v in daftar_bulan.items() if v == pilihan_nama][0]
    tahun_dipilih = c_thn.number_input("📅 Tahun:", value=sekarang_wib.year, min_value=2024, max_value=2030)
    ...
    try:
        ...
        # INISIALISASI REKAP (ANTI ERROR NOT DEFINED)
        rekap_harian_tim = {}
        rekap_total_video = {}
        if not df_f_f.empty:
        ...
        is_masa_depan = tahun_dipilih > sekarang_wib.year or (tahun_dipilih == sekarang_wib.year and bulan_dipilih > sekarang_wib.month)
        ...
        if not is_masa_depan:
            for _, s in df_staff_real.iterrows():
                ...
                _, _, pot_sp_real, _, _ = hitung_logika_performa_dan_bonus(df_t_staf, df_a_staf, bulan_dipilih, tahun_dipilih, level_target=lv_asli)
                ...
                if bulan_dipilih == sekarang_wib.month:
                    total_gaji_pokok_tim += (gaji_nett / 25) * min(sekarang_wib.day, 25)
                else:
                    total_gaji_pokok_tim += gaji_nett

        # --- UI: FINANCIAL COMMAND CENTER ---
        with st.expander("💰 ANALISIS KEUANGAN & KAS", expanded=False):
            total_out_riil = total_gaji_pokok_tim + float(bonus_terbayar_kas) + float(ops)
            m1, m2, m3, m4 = st.columns(4)
            m1.metric("💰 INCOME", f"Rp {float(inc):,.0f}")
            m2.metric("💸 OUTCOME", f"Rp {total_out_riil:,.0f}")
            m3.metric("📈 SALDO BERSIH", f"Rp {float(inc) - total_out_riil:,.0f}")
            margin = ( (float(inc) - total_out_riil) / float(inc) * 100) if inc > 0 else 0
            m4.metric("📊 MARGIN", f"{margin:.1f}%")
            st.divider()
            col_in, col_log_kas, col_v_kas = st.columns([1, 1.2, 1])
            with col_in:
                with st.form("form_kas_new", clear_on_submit=True):
                    f_tipe = st.pills("Tipe", ["PENDAPATAN", "PENGELUARAN"], default="PENGELUARAN")
                    f_kat = st.selectbox("Kategori", ["YouTube", "Brand Deal", "Gaji Tim", "Operasional", "Lainnya"])
                    f_nom = st.number_input("Nominal", min_value=0, step=50000)
                    f_ket = st.text_area("Keterangan", height=65)
                    if st.form_submit_button("🚀 SIMPAN"):
                        if f_nom > 0:
                            d_kas = {"Tanggal": sekarang_wib.strftime('%Y-%m-%d'), "Tipe": f_tipe, "Kategori": f_kat, "Nominal": str(int(f_nom)), "Keterangan": f_ket, "User": st.session_state.get("user_nama", "ADMIN").upper()}
                            supabase.table("Arus_Kas").insert(d_kas).execute()
                            sh.worksheet("Arus_Kas").append_row(list(d_kas.values()))
                            st.success("Saved!"); time.sleep(1); st.rerun()

        # ======================================================================
        # --- 4. RADAR PERFORMA TIM ---
        # ======================================================================
        st.write(""); st.markdown("### 📡 RADAR PERFORMA TIM")
        kolom_card = st.columns(4)
        rekap_v_total, rekap_b_cair, rekap_h_malas = 0, 0, 0
        performa_staf = {}
        ...
        for idx, s in df_staff_real.reset_index().iterrows():
            ...
            b_lembur, u_absen, pot_sp_r, level_sp_r, h_lemah = hitung_logika_performa_dan_bonus(df_t_staf_r, df_a_staf_r, bulan_dipilih, tahun_dipilih, level_target=str(s.get('LEVEL')).upper())
            
            bonus_real = 0
            if not df_kas.empty:
                df_kas['NOM_INT'] = pd.to_numeric(df_kas['NOMINAL'], errors='coerce').fillna(0)
                bonus_real = df_kas[(df_kas['KATEGORI'].str.upper() == 'GAJI TIM') & (df_kas['KETERANGAN'].str.upper().str.contains(n_up)) & (pd.to_datetime(df_kas['TANGGAL']).dt.month == bulan_dipilih)]['NOM_INT'].sum()

            rekap_v_total += len(df_t_staf_r)
            performa_staf[n_up] = len(df_t_staf_r)
            h_cair = sum(1 for q in rekap_harian_tim.get(n_up, {}).values() if q >= 3) if rekap_harian_tim else 0

            with kolom_card[idx % 4]:
                with st.container(border=True):
                    st.markdown(f'<div style="text-align:center; background:#1d976c; border-radius:5px; padding:2px;"><b style="color:white;">{n_up}</b></div>', unsafe_allow_html=True)
                    st.write(f"🎬 {len(df_t_staf_r)} Vid | 📅 {h_cair} Cair")
                    st.write(f"🚩 {level_sp_r} | 💰 Rp {int(bonus_real):,}")

        # --- 5. RANGKUMAN KOLEKTIF ---
        with st.container(border=True):
            st.write("📊 RANGKUMAN KOLEKTIF TIM")
            c_r1, c_r2, c_r3 = st.columns(3)
            c_r1.metric("🎬 TOTAL VIDEO", f"{int(rekap_v_total)}")
            c_r2.metric("🎯 TARGET", f"{len(df_staff_real)*40}")
            c_r3.metric("👑 MVP", max(performa_staf, key=performa_staf.get) if performa_staf else "-")

        # ======================================================================
        # --- 6. RINCIAN GAJI & SLIP (FIXED PROTEKSI MARET) ---
        # ======================================================================
        with st.expander("💰 RINCIAN GAJI & SLIP", expanded=False):
            try:
                ...
                for idx, s in df_staff_real.reset_index().iterrows():
                    n_up = str(s.get('NAMA', '')).strip().upper()
                    # PROTEKSI: Filter hanya jika data tidak kosong
                    df_a_slip = df_a_f[df_a_f['NAMA'] == n_up].copy() if not df_a_f.empty else pd.DataFrame()
                    df_t_slip = df_f_f[df_f_f['STAF'] == n_up].copy() if not df_f_f.empty else pd.DataFrame()
                    
                    _, _, p_sp, lv_sp, h_l = hitung_logika_performa_dan_bonus(df_t_slip, df_a_slip, bulan_dipilih, tahun_dipilih, level_target=str(s.get('LEVEL')).upper())
                    
                    gapok = int(pd.to_numeric(str(s.get('GAJI_POKOK')).replace('.',''), errors='coerce') or 0)
                    tunj = int(pd.to_numeric(str(s.get('TUNJANGAN')).replace('.',''), errors='coerce') or 0)
                    
                    with kol_v[idx % 2]:
                        with st.container(border=True):
                            st.write(f"**{n_up}**")
                            st.write(f"Estimasi: Rp {max(0, gapok+tunj-p_sp):,}")
                            if st.button(f"📄 SLIP {n_up}", key=f"slip_{n_up}"):
                                st.info("Fitur Print Slip Aktif.")
            except Exception as e_slip:
                st.error(f"Gagal memuat Slip: {e_slip}")

        # --- 7. DATABASE AKUN AI ---
        with st.expander("🔐 DATABASE AKUN AI", expanded=False):
            st.write("Fitur Akun AI Aktif.")
